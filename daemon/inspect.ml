...
let rec inspect_os () =
  ...
  (* Check if the root filesystems is duplicated by btrfs snapshots.
   * This happens especially for SLES guests.
   *)
  eprintf "inspect_os: check_for_duplicated_btrfs_snapshots_of_root\n%!";
  let fses = check_for_duplicated_btrfs_snapshots_of_root fses in
...
and check_for_duplicated_btrfs_snapshots_of_root fses =
  let drop_if_duplicate = function
  ...
  List.filter (Fun.negate drop_if_duplicate) fses
