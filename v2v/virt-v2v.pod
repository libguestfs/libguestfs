...
The role's access to vCenter, Datacenter, Cluster (if used), Folder
(if used) and ESXi hypervisor must all be set to C<OK>.  If set to
C<READ_ONLY> or C<NO_ACCESS>, this will prevent virt-v2v from working.
(This is a bug in libvirt which may be resolved in a future version.)
